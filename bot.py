...
class BotMetrics:
    ...
    def __init__(self):
        ...
        # НЕ запускаем асинхронную задачу в конструкторе - это вызовет ошибку
        # Задачу запустим потом в async init
...
# Создаем экземпляр метрик - НЕ запускаем asyncio.create_task здесь!
...
# Обработчик для webhook - исправлен
